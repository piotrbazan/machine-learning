...
def safe_random(i, margin, delta, size):
    while True:
        candidate = int(np.random.uniform(-180, 180))
        if abs(i - candidate) > margin and i + candidate - delta > 0 and i + candidate + delta < size:
            return candidate
